...
- Компактные статус-плашки в шапке:
  - время последнего обновления данных
  - время последней загрузки в браузере
  - остаток GitHub API и время сброса лимита
- Встроенные SVG-иконки для ссылок на репозиторий, карточек статистики, вкладок и строк настроек.
...
- Настройки страницы (иконка шестеренки):
  ...
  - режим темы (`System` по умолчанию, дополнительно `Light` / `Dark`)
  - локальные пороги для невзаимных подписок, неактивности друзей и окна данных
  - локальный список исключений
- Поведение темы:
  - по умолчанию интерфейс следует системной теме ОС/браузера
  - явный выбор `Light` / `Dark` сохраняется в `localStorage` браузера
